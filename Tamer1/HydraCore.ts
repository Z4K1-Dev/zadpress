...
export abstract class HydraCore extends BasePlugin {
  ...
  private async removeCoreRoutes(): Promise<void> {
    ...
    for (const route of routes) {
      ...
      this.getHookManager().addHook('hydra:removeRoute', () => ({
        path: route.path,
        method: route.method,
        core: this.coreType
      }));
  ...
  private async setupCoreModels(): Promise<void> {
    ...
    for (const model of models) {
      ...
      this.getHookManager().addHook('hydra:registerModel', () => ({
        name: model.name,
        schema: model.schema,
        core: this.coreType
      }));
  ...
  private async cleanupCoreModels(): Promise<void> {
    ...
    for (const model of models) {
      ...
      this.getHookManager().addHook('hydra:removeModel', () => ({
        name: model.name,
        core: this.coreType
      }));
   ...
   * Cleanup core features
   */
  protected abstract cleanupCoreFeatures(): Promise<void>;
}
